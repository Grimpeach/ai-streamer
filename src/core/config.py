...
class STTSettings(BaseModel):
    ...
    #: Прогреть энкодер при старте. Тишина + VAD не считается: тогда CUDA
    #: кернелы не вызываются и отсутствующий cuBLAS всплывает на первой реплике.
    warmup: bool = True
    #: Если CUDA-библиотек нет — перезагрузить модель на CPU, а не ронять PTT.
